...
import com.udawos.pioneer.effects.BannerSprites.Type;
...
import com.udawos.pioneer.windows.WndClass;
...
public class StartScene extends PixelScene {
	...
	public void create() {
		...
		curClass = null;
		updateClass( HeroClass.values()[Pioneer.lastClass()] );
	...
	/*private class ChallengeButton extends Button {
		...
		protected void createChildren() {
			...
			image = Icons.get( Pioneer.challenges() > 0 ? Icons.CHALLENGE_ON :Icons.CHALLENGE_OFF );
			add( image );
		}
		...
		protected void onClick() {
			if (Badges.isUnlocked( Badges.Badge.VICTORY )) {
				StartScene.this.add( new WndChallenges( Pioneer.challenges(), true ) {
					public void onBackPressed() {
						super.onBackPressed();
						image.copy( Icons.get( Pioneer.challenges() > 0 ?
