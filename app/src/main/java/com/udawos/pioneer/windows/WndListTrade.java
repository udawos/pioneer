...
//Buy items from a list
public class WndListTrade extends WndTabbed {
    ...
    private ArrayList<ListItem> items = new ArrayList<WndListTrade.ListItem>();
    ...
    public WndListTrade() {
        ...
        boolean showPotions = WndListTrade.showPotions;
        Tab[] tabs = {
                new LabeledTab( TXT_POTIONS ) {
                    protected void select( boolean value ) {
                        super.select( value );
                        WndListTrade.showPotions = value;
                        updateList();
                    };
                },
    ...
    private void updateList() {
        ...
        txtTitle.text( Utils.format( TXT_TITLE, showPotions ? TXT_POTIONS : TXT_AMMO ) );
        txtTitle.measure();
        ...
        float pos = 0;
        for (Class<? extends Item> itemClass : showPotions ? Potion.getKnown() : Ammo.getKnown()) {
        ...
        for (Class<? extends Item> itemClass : showPotions ? Potion.getUnknown() : Ammo.getUnknown()) {
            ListItem item = new ListItem( itemClass );
            item.setRect( 0, pos, width, ITEM_HEIGHT );
            content.add( item );
            items.add( item );
