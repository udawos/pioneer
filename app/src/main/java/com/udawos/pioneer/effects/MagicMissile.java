...
import com.udawos.pioneer.effects.particles.WoolParticle;
...
public class MagicMissile extends Emitter {
	...
	public static void greyLight( Group group, int from, int to, Callback callback ) {
		MagicMissile missile = ((MagicMissile)group.recycle( MagicMissile.class ));
		missile.reset( from, to, callback );
		missile.size( 1 );
		missile.pour( GreyParticle.MISSILE, 0.01f );
	}
