...
import com.udawos.pioneer.effects.Degradation;
import com.udawos.pioneer.effects.Speck;
import com.udawos.pioneer.items.armor.Armor;
import com.udawos.pioneer.items.bags.BackpackMid;
import com.udawos.pioneer.items.bags.BackpackTop;
...
public class Item implements Bundlable {
    ...
    private static final String TXT_PACK_HEAVY	= "Your load is too heavy for the %s";
	...
	public boolean collect( Bag container ) {
        ...
        //if (Dungeon.weight >= 50) {
        //    GLog.n(TXT_PACK_HEAVY, name());
        //    return false;
       // }
        ...
        for (Item item:items) {
            if (item instanceof Bag && ((Bag)item).grab( this )) {
                ...
                else if  (item instanceof BackpackMid && ((BackpackMid)item).grab( this )) {
                    return collect( (BackpackMid)item );}
                else if  (item instanceof BackpackTop && ((BackpackTop)item).grab( this )) {
                    return collect( (BackpackTop)item );
			}
		}
		...
		if (stackable) {
			...
			for (Item item:items) {
				if (item.getClass() == c) {
                    ...
                    //Dungeon.weight = Dungeon.weight + weight();
  ...
  //trying to detach ammo components when used
  //  public Item detachthis() {return detachAll(Dungeon.hero.belongings.backpack);
   // }
